...
- **The sheet animates out, which `animation` already promised.** The option was documented as
  covering entry and exit, but only the entry had keyframes: the sheet vanished the instant it
  settled while the backdrop it sat on faded in on arrival. The closing pass replays the same
  keyframes in reverse and the view is destroyed once they finish. Nothing waits on it that the
  caller can see — the promise from `open()` has already resolved by then — and when there is no
  animation to wait for, because the option is off or the reader asked for reduced motion, the
  sheet leaves at once as before.
